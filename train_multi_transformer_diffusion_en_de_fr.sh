...
data_path=hdfs://haruna/home/byte_arnold_hl_mlnlc/user/songzhenqiao/multilingual_glat/total_data/wmt_en_de_fr
local_dataset_path=${data_path}

# Diffusion data path
diffusion_data_path=${data_path}/new_kd_keep_bpe/new_better_diffusion_data
...
remote_checkpoint_path=${hdfs_checkpoint_path}/wmt_en_de_fr_diffusion
mkdir -p ${local_checkpoint_path}
hadoop fs -mkdir -p ${hdfs_checkpoint_path}
hadoop fs -mkdir -p ${remote_checkpoint_path}

# initialize model path
initialized_model_path=${hdfs_checkpoint_path}/wmt_en_de_fr
