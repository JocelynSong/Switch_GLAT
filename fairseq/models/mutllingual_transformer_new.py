...
@register_model("multilingual_transformer_song")
class MultilingualTransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return MultilingualTransformerEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return MultilingualTransformerDecoder(
            args,
            tgt_dict,
            embed_tokens,
    ...
    def forward_decoder(self, prev_output_tokens, src_lang=None, tgt_lang=None, **kwargs):
        return self.decoder(prev_output_tokens, src_lang=src_lang, tgt_lang=tgt_lang, **kwargs)
...
class MultilingualTransformerEncoder(FairseqEncoder):
    """
    ...
    def forward_scriptable(
        ...
        x, encoder_embedding = self.forward_embedding(src_tokens, token_embeddings, src_lang)
    ...
    def forward_torchscript(self, net_input: Dict[str, Tensor], src_lang=None):
        ...
        if torch.jit.is_scripting():
            ...
            return self.forward_non_torchscript(net_input, src_lang)

    @torch.jit.unused
    def forward_non_torchscript(self, net_input: Dict[str, Tensor], src_lang=None):
        encoder_input = {
            k: v for k, v in net_input.items() if k != "prev_output_tokens"
        }
        return self.forward(**encoder_input, src_lang=src_lang)
...
class MultilingualTransformerDecoder(FairseqIncrementalDecoder):
    """
...
@register_model_architecture("multilingual_transformer_song", "multilingual_transformer_song_base")
...
@register_model_architecture("multilingual_transformer_song", "multilingual_transformer_song_wider")
def transformer_vaswani_wmt_en_de_big(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 1024)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4096)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 16)
