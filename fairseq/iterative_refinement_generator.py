...
class MultilingualIterativeRefinementGenerator(object):
    ...
    @torch.no_grad()
    def generate(self, models, sample, prefix_tokens=None, src_lang=None, tgt_lang=None):
        # TODO: iterative refinement generator does not support ensemble for now.
        if not self.retain_dropout:
            for model in models:
                model.eval()
        ...
        # initialize
        encoder_out = model.forward_encoder([src_tokens, src_lengths], src_lang=src_lang, tgt_lang=tgt_lang)
        prev_decoder_out = model.initialize_output_tokens(encoder_out, src_tokens)
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder(
                prev_decoder_out, encoder_out, src_lang=src_lang, tgt_lang=tgt_lang, **decoder_options
            )
    ...
    @torch.no_grad()
    def generate_diffusion_data(self, models, sample, prefix_tokens=None, src_lang=None, tgt_lang=None,
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder_diffusion_data(
                prev_decoder_out, encoder_out, src_lang=src_lang, tgt_lang=tgt_lang, diffusion_lang=diffusion_lang,
